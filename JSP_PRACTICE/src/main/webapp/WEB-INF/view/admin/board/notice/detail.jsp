                ...
                <div class="margin-top first">
                    ...
                    <table class="table">
                        <tbody>
                            ...
                            <tr>
                                <th>작성일</th>
								<td class="text-align-left text-indent" colspan="3"><format:formatDate pattern="yy-MM-dd hh:mm:ss" value="${n.regdate}"></format:formatDate></td>
                            </tr>
                            <tr>
                                <th>작성자</th>
                                <td>${n.writerId }</td>
                            ...
                            <tr>
								...
								<td colspan="3"><c:forTokens var="fileName"
										...
										<c:if test="${fn:endsWith(fileName, '.zip') }"> <!-- 만약 끝에 .zip이 포함되어있으면 style 변경 -->
											<c:set var="style" value="font-weight: bold; color:red;" />
										</c:if>
										<a download href="/upload/${fileName}" style="${style}">${fn:toUpperCase(fileName)}</a> <!-- 우리가 지정한 절대경로로 맞춰주기 download 속성 활용-->
										<c:if test="${!st.last}"><!-- 마지막 토큰이 아니면 /로 구분짓기 -->
											/
										</c:if>
									</c:forTokens></td>
							</tr>
